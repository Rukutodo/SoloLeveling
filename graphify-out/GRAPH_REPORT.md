# Graph Report - SoloLeveling  (2026-05-21)

## Corpus Check
- 98 files · ~49,855 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 530 nodes · 716 edges · 77 communities (36 shown, 41 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 13 edges (avg confidence: 0.93)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9dbb2c5f`
...
1. `dbConnect()` - 72 edges
2. `awardXP()` - 21 edges
3. `User` - 15 edges
4. `sendSystemMessage()` - 10 edges
5. `useNotifications()` - 7 edges
6. `XP_REWARDS` - 7 edges
7. `Core Features` - 7 edges
...
- `GET()` --calls--> `dbConnect()`  [EXTRACTED]
  app/api/friends/status/route.ts → lib/mongodb.ts
- `GET()` --calls--> `dbConnect()`  [EXTRACTED]
...
## Communities (77 total, 41 thin omitted)

### Community 0 - "Core API Routes & Models"
Cohesion: 0.05
Nodes (55): GET(), GET(), GET(), DELETE(), GET(), POST(), DELETE(), GET() (+47 more)

### Community 1 - "Architectural Mapping & Features"
Cohesion: 0.07
Nodes (38): calculateBMI(), GET(), POST(), GET(), DELETE(), GET(), POST(), POST() (+30 more)
...
Cohesion: 0.1
Nodes (13): GET(), POST(), GET(), FoodEntrySchema, IFoodEntry, IInvestment, InvestmentSchema, IMainQuest (+5 more)
...
Cohesion: 0.14
Nodes (7): branding, iconSets, navConfig, sectionLabels, SidebarProps, Note, Todo
...
Cohesion: 0.17
Nodes (8): metadata, ThemeProvider(), Toast, toastConfig, ToastContext, ToastContextType, ToastProvider(), ToastType

### Community 10 - "Auth Configuration & Monitoring"
Cohesion: 0.17
Nodes (9): FriendSchema, IFriend, ITodo, TodoSchema, GET(), DELETE(), GET(), POST() (+1 more)
...
Cohesion: 0.25
Nodes (9): Sidebar(), ThemeContext, ThemeContextType, ThemeInfo, ThemeName, THEMES, useTheme(), ThemeSwitcher() (+1 more)

### Community 13 - "Investment Management"
Cohesion: 0.31
Nodes (7): NotificationContext, NotificationContextType, NotificationProvider(), useNotifications(), useToast(), TopHeader(), HunterNetwork()
...
- **Why does `dbConnect()` connect `Core API Routes & Models` to `Architectural Mapping & Features`, `Auth Configuration & Monitoring`, `Quests & Physical Metrics`, `Mongoose Models & Schemas`?**
  _High betweenness centrality (0.048) - this node is a cross-community bridge._
- **Why does `awardXP()` connect `Architectural Mapping & Features` to `Core API Routes & Models`, `Auth Configuration & Monitoring`?**
